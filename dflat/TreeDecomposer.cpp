...
DecompositionPtr TreeDecomposer::decompose(const Instance& instance) const
{
	...
	if(optEliminationOrdering.getValue() == "min-degree")
		...
		//htd::OrderingAlgorithmFactory::instance().setConstructionTemplate(new htd::MinDegreeOrderingAlgorithm());
		//htd::OrderingAlgorithmFactory::instance().setConstructionTemplate(new htd::NaturalOrderingAlgorithm());
...
/**/
	...
	if(optEliminationOrdering.getValue() == "min-degree")
	{
	htd::IOrderingAlgorithm* ordering = new htd::MaximumCardinalitySearchOrderingAlgorithm();
	std::vector<htd::vertex_t> ordr;
	ordering->writeOrderingTo(graph.internalGraph(), ordr);
	delete ordering;
